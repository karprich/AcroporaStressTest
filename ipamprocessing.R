...
#mod day 32
mod_TT <- lm(Y~Genotype, data=all.f[which(all.f$Date=="2017-06-23"),])
mod_TT <- lm(Y~Tank, data=all.f[which(all.f$Date=="2017-06-23"),])
anova(mod_TT)
TukeyHSD(aov(mod_TT))
cld(lsmeans(mod_TT, "Tank"))
mod_Tank_all <-lm(Y ~ Tank, data= all.f)
mod_Geno <- lm(Y~Genotype, data = all.f)
